...
class MFFunction(object):
    ...
    def gradient(self, Z, X, delays):
        """gradient of X, Z"""
        f = self.eval_at_fidel_single_point_normalised(Z, X)
        time.sleep(delays[0])
        delays = np.delete(delays, 0)
        delta = 0.05
        X_grad = np.zeros(len(X))
        for i in range(len(X)):
            X_i_perturb = np.zeros(len(X))
            X_i_perturb[i] = delta
            X_i = X + X_i_perturb
            f_i = self.eval_at_fidel_single_point_normalised(Z, X_i)
            time.sleep(delays[0])
            delays = np.delete(delays, 0)
            X_grad[i] = (f_i - f) / delta
        Z_grad = np.zeros(len(Z))
        for i in range(len(Z)):
            Z_i_perturb = np.zeros(len(Z))
            Z_i_perturb[i] = delta
            Z_i = Z + Z_i_perturb
            f_i = self.eval_at_fidel_single_point_normalised(Z_i, X)
            time.sleep(delays[0])
            delays = np.delete(delays, 0)
            Z_grad[i] = (f_i - f) / delta
        # f_grad = np.concatenate([Z_grad, X_grad])
        print("X_grad:", X_grad)
        print("Z_grad:", Z_grad)
        print("value:", f)
        return Z_grad, X_grad, f
