...
mf_gp_bandit_args = [
  ...
  get_option_specs('function', True, None,
...
#     self._monitor_opt_val()
  ...
  def query(self, fidel_pt, domain_pt):
    ...
    current_delay = self.delays[self.delay_num]
  ...
  def optimise(self, max_capital):
    ...
    # Main loop --------------------------
    # while (current_time - start_time) < max_capital:
...
#     raise ValueError('mf_opt_func should be a mf_func.MFOptFunction instance.')
  if acq is not None:
    if options is None:
      ...
      options = load_options(all_mf_gp_bandit_args, reporter=reporter)
    options.acq = acq
  # invoke the optimization function here
