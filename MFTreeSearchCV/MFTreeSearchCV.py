...
class MFTreeSearchCV(GridSearchCV):
    ...
    def __init__(self, estimator, param_dict, fidelity_range, total_budget, policy, delay_type, scoring='accuracy', \
    ...
    def _create_mfobject(self, X, y):
        MF = MFTreeFunction(X, y, self.estimator, self.param_dict, \
                            self.fidelity_range, self.scoring, self.greater_is_better, self.fixed_params, \
                            self.n_jobs, self.cv)
    ...
    def fit(self, X, y):
        ...
        self.MP = MFPOO(mfobject=self.MF, nu_max=self.nu_max, rho_max=self.rho_max, \
                        ...
                        mult=self.mult, policy=self.policy, delay_type=self.delay_type, tol=self.tol,
